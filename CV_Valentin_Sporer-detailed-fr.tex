...
\begin{entrylist}
	\entry
		...
		{Tenable}
		{
            ...
            \begin{itemize}
				\item Lead dev d'un projet visant à simplifier et accélérer, pour les utilisateurs, la configuration des capacités de détection d'attaques (\texttt{Rust}, \texttt{C\#/.NET}, \texttt{SQL}, \texttt{TypeScript/React})
				\item Lead dev d'un projet permettant l'activation et la désactivation automatique des capacités de détection d'attaques, basées sur des statistiques en temps réel (\texttt{Rust}, \texttt{C\#/.NET}, \texttt{TypeScript/React})
				\item Développement du scaling horizontal des services de détection d'attaques (\texttt{C\#/.NET})
				\item Maintien et développement d'APIs JSON et requêtes SQL associées (\texttt{TypeScript/Node.js}, \texttt{Kotlin}, \texttt{SQL})
				\item Développement de composants React et de pages pour SPA (\texttt{TypeScript/React})
            \end{itemize}
		}
	\entry
		...
		{Alsid / Tenable}
		{
			Maintien et amélioration de code C, C++, C\#, TypeScript (Node.JS), SQL sur la plupart des services back-end. Ecriture en Rust d'un agent de collecte d'évènement Windows.\\
